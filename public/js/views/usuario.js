            ...
            success: function (data, response, options) {
                  ...
                  $('#novousuario').click(function() {
                      ...
                      $('.usuarioeditablenew').editable('submit', {
                        ...
                        //     data.value = CryptoJS.SHA3(params.value, { outputLength: 512 }).toString(CryptoJS.enc.Base64);
                        //     return params;
                        // },
                        data: {
                            senha: function() {
                                return CryptoJS.SHA3($('#usuario-senha').text(), { outputLength: 512 }).toString(CryptoJS.enc.Base64);
                            }
                        },
                        ajaxOptions: {
                          dataType: 'json'
...
window.configura_UsuarioEditable_BtnApagar = function() {
  ...
  $('.usuarioeditable').editable({
      ...
      params: function(params) {
        if (params.name == 'senha') {
          params.value = CryptoJS.SHA3(params.value, { outputLength: 512 }).toString(CryptoJS.enc.Base64);
        } else {
          params.value = unescape(encodeURIComponent(params.value));
        }
        return params;
      },
      ajaxOptions: {
